...
#29 November - added ramdisk support to speed up conversions
#30 November - add coverted file flag in folder following successful conversion so can skip next time

#ideas
# put m4b into folders before processing if they are at the top level directory
# write out a generic cover.jog file if one does not exist : https://stackoverflow.com/questions/75245060/is-there-a-way-of-attaching-an-image-on-a-python-code-in-such-a-way-that-it-beco
# execution time into minutes
...
def print_header ():
    ...
    print(" ramdisk version              |_|        ")
    print(f"{bcolors.OKBLUE} Vesion: " + last_update + f"{bcolors.ENDC}")
    print("")



def getBookInfo(ram_disk_file, ram_disk_path, number_of_m4b, number_of_m4b_remaining): #run the ffmpeg command on the m4b file and pipe the output to a text file
    if display_function_name:
        print_function_name(inspect.stack()[0][3])

    #book_info_location = (ram_disk_path + "/" + "BookInfo.txt")
    command = ffmpeg_path + ' -i "' + ram_disk_file + '" -hide_banner > ' + bookInfoFileLocation + ' 2>&1'
    ...
    print (padding + "Parsing audio from : " + ram_disk_file)
...
#call with ram_disk_m4b_file as file variable
def parseBook(dir_path, file_name, file, overwrite_files, stop_conversion):
    if display_function_name:
        print_function_name(inspect.stack()[0][3])
    ...
    if numberOfChapters-1 == 0:
        ...
        if os.path.exists(output_path) and not overwrite_files:
    ...
    if numberOfChapters-1 > 0:
        ...
        for chapter, start, end in rex.findall(bookInfoContents): #use the regex to find each chapter information from the bookContents information
            ...
            # if there are already mp3s which will get overwritten and you did not pass the "-replace" then exit
            if os.path.exists(output_path) and stop_conversion:
                print (padding + ".mp3 files exist in destination folder and you don't want to overwrite.")
                exit()

            if os.path.exists(output_path) and not overwrite_files:
                print(padding + 'File exists. Skipping: ' + fileNumberPrefix + ' - ' + file_name + '.mp3')
                sleeping(2)
                

            if not os.path.exists(output_path) or overwrite_files:
            	# Convert
                ...
                chapter_elapsed_time = round(chapter_end_time - chapter_start_time,2)
                print(' : Execution time:', chapter_elapsed_time, 'seconds')
...
def removeFile(bookFileLocation): #remove the file passed in bookFileLocation
    ...
    time.sleep(5) # sleep to allow the file to be removed
...
def mount_ramdisk(ram_disk_path):
    if not os.path.ismount(ram_disk_path):
        print (padding + "warning : ramdisk does not exist : " + ram_disk_path)
        print(padding + "Trying to mount : " + ram_disk_path)
        try:            
            subprocess.run (["mount", ram_disk_path], capture_output=True)
        except:
            print(padding + "Error : Unable to mount : " + ram_disk_path)            
            print(padding + 'Add : "tmpfs /media/ramdisk tmpfs rw,nodev,nosuid,users,noauto,size=2G" to /etc/fstab')        
            print("")
            exit()
    ...
    if os.path.ismount(ram_disk_path):
        print (padding + "OK : ramdisk is mounted : " + ram_disk_path)
        print("")


def write_success_converted_file(dir_path, already_successfully_converted_filename):
    #write file to orginal source location
    f = open(dir_path + already_successfully_converted_filename, "w")
    f.write("m4b2mp3 has converted this folder.")
    f.close()


def check_for_write_success_converted_file(dir_path, already_successfully_converted_filename):
    #if not os.path.exists(dir_path + already_successfully_converted_filename):
...
base_dir = os.getcwd() # the directory contains the audiobooks - the current directory
padding = "  "
ram_disk_path = "/media/ramdisk"
bookInfoFileLocation = (ram_disk_path + "/BookInfo.txt") # location file to store the information about each audiobook
already_successfully_converted_filename = "/m4b2mp3_converted.txt"
...
def main():
    ...
    #check if the ramdisk has been mounted
    mount_ramdisk(ram_disk_path)
    ...
    if not count_only:        
        if overwrite_files and remove_m4b:
        ...
        if overwrite_files and not remove_m4b:
            print(padding + "I will " + f"{bcolors.FAIL}replace " + f"{bcolors.ENDC}any existing .mp3's and will " + f"{bcolors.OKGREEN}keep" + f"{bcolors.ENDC} the original .m4b")

        if not overwrite_files and remove_m4b:        
        ...
        if not overwrite_files and not remove_m4b:        
            print(padding + "I will " + f"{bcolors.OKGREEN}not replace " + f"{bcolors.ENDC}any existing .mp3's and will " + f"{bcolors.OKGREEN}keep" + f"{bcolors.ENDC} the original .m4b")
    ...
    for file in file_list: #for each file to be converted
        ...
        #check if write_success_converted_file is present
        #this means the folder has already been converted
        folder_already_converted = check_for_write_success_converted_file(dir_path, already_successfully_converted_filename)

        if folder_already_converted:            
            print(padding + "Information: " + dir_path + " has already been converted, remove the " + already_successfully_converted_filename + " to allow reprocessing")
            sleeping (5)

        if not folder_already_converted:
                    
            #copy from the source to the ramdisk
            ram_disk_m4b_file = (ram_disk_path + "/" + file_name + ".m4b")
            print ("")
            print(padding + "ramdisk copy .m4b to " + ram_disk_m4b_file)
            shutil.copyfile(file, ram_disk_m4b_file)
            
            #start useing the m4b in ramdisk
            getBookInfo(ram_disk_m4b_file,ram_disk_path, number_of_m4b, number_of_m4b_remaining) #get the m4b information
            #dir_path, file_name = os.path.split(file) # split the m4b file location into path and filename
            #file_name = os.path.splitext(file_name)[0] # Drop the file extension so we can use mp3
                        
            parseBook(dir_path, file_name, ram_disk_m4b_file,overwrite_files, stop_conversion) # convert the book to mp3
            removeFile(bookInfoFileLocation) # remove the text information file
            removeFile(ram_disk_m4b_file) # remove the ramdisk m4b file

            if remove_m4b: # if requested to remove from command line
                removeFile(file) # remove the original m4b file

            #add conversion success file to allow skipping of re converting
            write_success_converted_file(dir_path, already_successfully_converted_filename)

            sleeping(5) # sleep for 5 seconds
